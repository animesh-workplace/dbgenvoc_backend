...
Enhanced aggregation API with genomic position and pathway filtering.

Features:
- Standard aggregations (count, sum, avg, min, max, distinct_count)
...
class AggregationResponse(BaseModel):
    ...
    total_records: int  # Total records after WHERE filtering (including genomic)
...
async def generic_aggregate(
    ...
    """
    Enhanced generic aggregation endpoint with genomic position filtering.
    ...
    Example 1: Count variants by gene in genomic range
    {
      "column": "gene",
    ...
    Example 2: Count variants at multiple positions
    {
    ...
    Example 3: Genes with >10 variants in pathway
    {
      "column": "gene",
      "aggregation_type": "count",
      "group_by": ["gene"],
    ...
    Example 4: Combined filters - missense in TP53 region
    {
    ...
    try:
        ...
        # 2b. Apply Genomic Position Filters (NEW)
        query = _apply_genomic_position_filter(query, model_class, request.genomic_filter)

        # 3. Capture Total Records (After WHERE + genomic filters, Before GROUP BY)
...
Response:
{
  ...
  "groups_before_having": 3,
  "groups_after_having": 3,
  "result": [
    {"gene": "TP53", "aggregated_value": 142},
    {"gene": "WRAP53", "aggregated_value": 10},
    {"gene": "LOC100507053", "aggregated_value": 4}
  ]
}
...
Example 3: Genes with >10 variants in PI3K-AKT pathway
-------------------------------------------------------
{
  "table": "nibmg_exome_somatic_variants",
  "column": "gene",
...
Example 4: Top 10 missense mutations in TP53 region
----------------------------------------------------
{
  "table": "nibmg_exome_somatic_variants",
  "column": "protein_change",
...
Example 5: Percentage of variants by class in specific region
--------------------------------------------------------------
{
  "table": "nibmg_exome_somatic_variants",
  "column": "variant_class",
...
Example 6: Average position by gene in pathway
-----------------------------------------------
{
  "table": "nibmg_exome_somatic_variants",
  "column": "start",
