...
aggregate_agent = Agent(
    ...
    system_message=f"""
        ...
        **HAVING Clause - Filter Aggregated Results (IMPORTANT)**
        The `having` field filters groups AFTER aggregation. Use it for queries that require:
        - Finding entities with specific aggregate counts
        - Filtering by thresholds on aggregated values
        - Identifying co-occurrence patterns (e.g., mutations in BOTH gene X AND gene Y)
        ...
        **Query Context Parsing Rules**
        ...
        3. **Filters**: Gene names, variant types, disease types, etc. (goes in `filters`)
        4. **Grouping**: If asking for breakdown/distribution (goes in `group_by`)
        5. **Aggregate Filtering**: Threshold/count requirements (goes in `having`)
        6. **Calculation**: Count, percentage, distinct count, etc.
