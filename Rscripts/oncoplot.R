...
prepare_maf_data = function(table_name, database_path = "database/database.sqlite3") {
  # Connect to database
  con = dbConnect(SQLite(), database_path)
  on.exit(dbDisconnect(con))
  
  # Read table
  maf_df = dbReadTable(con, table_name)
...
generate_oncoplot_matrix = function(table_name, maf_df) {
  ...
  oncoplot_matrix = maftools::oncoplot(
    maf = maf_obj,
    top = length(available_genes),
  )
  
  output_file = paste0(table_name, "_oncoplot_matrix.tsv")
  write.table(oncoplot_matrix$oncomatrix, output_file, sep="\t", quote=FALSE)
}
